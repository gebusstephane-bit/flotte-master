-- ============================================================================
-- MIGRATION COMPLÈTE : Corrections Inspection Form
-- Date: 2026-01-31
-- Description: Ajout des 3 niveaux carburant + fix défauts critiques
-- ============================================================================

-- 1. AJOUTER LES COLONNES CARBURANT MANQUANTES
-- ----------------------------------------------------------------------------
...
-- 3. METTRE À JOUR LA VUE ANALYTIQUE
-- ----------------------------------------------------------------------------
...
comment on column vehicle_inspections.defects is 'JSON array: [{category, severity, description, location, photo_url, reported_at}] - severity: critical|warning|minor';

-- 5. VÉRIFICATION
-- ----------------------------------------------------------------------------
select 
  'Colonnes carburant:' as verification,
  count(*) filter (where column_name = 'fuel_gasoil') as fuel_gasoil,
  count(*) filter (where column_name = 'fuel_gnr') as fuel_gnr,
  count(*) filter (where column_name = 'fuel_adblue') as fuel_adblue
